...
const {productsCarr, eliminar} = useProducts()
        ...
        <div className=' flex flex-wrap  bottom-0 justify-center  bg-blue-500 w-screen'>
            {productsCarr.map((product) => (
                <div key={product.id} className='flex justify-center shadow-md shadow-gray-600 border border-gray-500 min-w-2/12 max-w-3/5 bg-white m-2 gap-2 rounded p-1'>
                    ...
                    <button onClick={()=>eliminar(product.id)} className=" m-1 w-5 h-5 flex items-center justify-center rounded-full  active:scale-90 transition-all duration-150 ease-in-out cursor-pointer">
                        ❌
                    </button>
                </div>
            ))}
